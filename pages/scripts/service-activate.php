...
if (isset($_GET['service_id']) && isset($_SESSION['provider_id'])) {
    ...
    $query = "UPDATE services SET status = 1 WHERE service_id = ? AND provider_id = ?";
    ...
    if ($stmt->execute()) {
        $stmt->close();
        $conn->close();
        header("Location: ../provider-services.php?success=ServiceActivated");
        exit();
    } else {
        $stmt->close();
        $conn->close();
        header("Location: ../provider-services.php?error=ActivationFailed");
        exit();
    }
} else {
